...
$('.menu__item a, a.form-animation').on('click', function(_event) {
...
$(document).on('click', '.openGallery', function (){
    let tthis = this;
    BX.ajax.runAction('sp:tools.gallery.show', {
        data: {
            id: $(tthis).data('id')
        }
    }).then(function (response) {
        $('#ajaxGallery').html(response.data);
        popupOpen(document.getElementById('galeryPopUp'));
        //$('#galeryPopUp .gallery-popup-content__item').fancybox();
    }, function (response) {
        console.log(response);
    });
});

$(document).on('submit', '.sendFormComponent', function (){
    let tthis = this;
    BX.ajax.runAction('sp:tools.forms.send', {
        data: {
            params:  {
        ...
        popupOpen(document.getElementById('modalSuccess'));
        $(tthis)[0].reset();
    }, function (response) {
        console.log(response);
    });
